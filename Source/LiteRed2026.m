...
System`CheckUpdates["LiteRed`"]:=Module[{out},RunProcess[{"git", "remote","update"}];
out=RunProcess[{"git","-C",DirectoryName[$InputFileName],"diff", "origin/main","--name-only"},"StandardOutput"];
If[out=!="",If[!ChoiceDialog["'git diff origin/main --name-only' gives\n\n"<> out,WindowTitle->"Remote diverges from local."],Quit[]]];
]


If[FileExistsQ[DirectoryName[$InputFileName]<>"check_updates.md"],System`CheckUpdates["LiteRed`"]]
...
SawToothLine;WavyLine;CurlyLine;
...
adjugate=Symbol["System`Adjugate"],
adjugate=Transpose[MapIndexed[(-1)^Plus@@#2*#1&,Map[Reverse,Minors[#],{0,1}],{2}]]&
]
...
DistributePoints[pts_,p_]:=Module[{ls,l,npts,n,k=0,inds},
(*First, we find points on the curve*)
ls=Prepend[Accumulate[Norm/@Differences[pts]],0];
l=Last[ls];(*Total length*)
ls=ls/l;ls[[-1]]=1;
n=Ceiling[l/p];(*corrected period*)
(*indices of line segments where the points should appear*)
Prepend[MapIndexed[pts[[#1]]+l*(First[#2]/n-ls[[#1]])*Normalize[pts[[#1+1]]-pts[[#1]]]&,Flatten[MapIndexed[ConstantArray[First[#2],-Subtract@@{k,k=Floor[ #1*n]}]&,Rest@ls]]],pts[[1]]]
]


DistributePairs[pts_,p1_,p2_]:=Module[{ls,ls1,l,npts,n,k=0,f,inds,d1,d2},
(*First, we find points on the curve*)
ls=Prepend[Accumulate[Norm/@Differences[pts]],0];
l=Last[ls];(*Total length*)
ls=ls/l;ls[[-1]]=1;
n=Ceiling[(l-p1)/(p1+p2)];
{d1,d2}={p1,p2}/(n(p1+p2)+p1);(*corrected period*)  
ls1=Join[{0},Accumulate[Flatten[Table[{d1,d2},{n}]]]];
inds=Flatten[Function[s,FirstPosition[ls,_?(#>s&),0,{1}]-1]/@ls1];
Append[MapIndexed[pts[[#1]]+l*(ls1[[First[#2]]]-ls[[#1]])*Normalize[pts[[#1+1]]-pts[[#1]]]&,inds],pts[[-1]]]
]


SawToothLine::usage="SawToothLine[points,p,a,d] constructs a sawtooth path along the contour Line[points]. Here p is the period, d is the fraction of raising part of a tooth, a is an amplitude. When d is not given, it defaults to 0.5. When a is not given, it defaults to p.";


SawToothLine[pts_,p_,a_:Automatic,d_:1/2]:=Module[{npts,mpts,s=Replace[a,Automatic->p]},
npts=DistributePoints[pts,p]; 
(*Now we insert intermediate points*)
mpts=(s=-s;#)&[#1+#2+Normalize[Reverse[#2]]*{s,-s}]&@@@Transpose[{Most[npts],Differences[npts]*d}];
Line[Join[{npts[[1]]},mpts,{npts[[-1]]}]]
]


WavyLine::usage="WavyLine[points,p,a] constructs a wavy line path along the contour Line[points]. Here p is the period, a is an amplitude. When a is not given, it defaults to p.";


WavyLine[pts_,p_,a_:Automatic]:=Module[{npts,mpts,s=Replace[a,Automatic->p]},
npts=DistributePoints[pts,p]; 
(*Now we insert intermediate points*)
mpts=(s=-s;#)&[#1+#2+Normalize[Reverse[#2]]*{s,-s}]&@@@Transpose[{Most[npts],Differences[npts]/2}];
BSplineCurve[Riffle[npts,mpts]]
]


CurlyLine::usage="CurlyLine[points,f,b,a] constructs a wavy line path along the contour Line[points]. Here f is the length of the forward curl, b is that of the backward, a is an amplitude. When b is not given, it defaults to f/3. When a is not given, it defaults to f.";


CurlyLine[pts_,f_,b1_:Automatic,a_:Automatic]:=Module[{npts,mpts,s=Replace[a,Automatic->f],b=Replace[b1,Automatic->f/3],seq,t},
npts=DistributePairs[pts,f,-b]; 
(*Now we insert intermediate points*)
mpts=(t=Reverse[Normalize[#2-#1]]*{s,-s};seq[#1+t,#2+t])&@@@Transpose[{Most[npts],Rest[npts]}];
BezierCurve[Riffle[npts,mpts]/.seq->Sequence]
(*BSplineCurve[Join[pts[[{1}]],mpts,pts[[{-1}]]]/.seq->Sequence]*)
]
...
StyleBox[\"set\", \"TI\"]\)] defines bases which cover \!\(\*
StyleBox[\"set\", \"TI\"]\).";


NewDsBases::names:="The list of names for the bases, `1`, is too short.";
NewDsBases[nm_?DsSetQ,bs_List:Automatic,patt_:Automatic]:=Module[{nms,pat=Replace[patt,Automatic->SectorsPattern[nm]],matr,bases,ii,iid,secpat,cutds,k,ds=Ds[nm],ns,nbs},
ns=$NamingFunction[Length[ds]];
matr=Outer[Coefficient,LFDistribute[ds,sp],SPs[nm]];
bases=Rest/@List@@@DeleteDuplicates[jSector/@jVars[PFReduce/@(j@@@BiggestSectors[nm])]];
nbs=Length[bases];
Quiet[Check[nbs=Take[Replace[bs,Automatic:>Array[ToExpression[ToString[nm]<>IntegerString[#,10,Ceiling[Log10[nbs+1]]]]&,{nbs}]],nbs],Message[NewDsBases::names,bs];Abort[]],Take::take];
bases=(ii=iid=Flatten[Position[#,1,{1}]];MapIndexed[If[MatrixRank[matr[[Join[ii,#2]]]]>Length[ii],ii=Join[ii,#2]]&,Flatten[Position[#,0,{1}]]];ii=Sort[ii];
secpat=Replace[ii,Append[Thread[iid->_],_->0],{1}];
cutds=CutDs[nm][[ii]];
{ii,secpat,cutds}
)&/@bases;
NewDsBasis@@@MapThread[{#1,ds[[#2[[1]]]],LMs[nm],Directory->BasisDirectory[nm], SectorsPattern->#2[[2]],CutDs->#2[[3]]}&,{nbs,bases}];
(*rules*)
SetToBasesRule[nm]^=MapThread[((j[nm]@@Replace[MapAt[p[#,_]&,ns,List/@#2],Except[_p]->0,{1}]/.p:>Pattern):>Evaluate[j[#1]@@ns[[#2]]])&,{nbs,bases[[All,1]]}];
...
GBasisHack[polys_,vars_,MonomialOrder->m_]:=Module[{l=Length@vars,fvars=Join[vars,Complement[Variables[polys],vars]],mf},mf=IdentityMatrix[Length@fvars];mf[[;;l,;;l]]=m;GroebnerBasis[polys,fvars,MonomialOrder->mf]]
...
StyleBox[\"basis\", \"TI\"]\)] gives a list of all parameters of \!\(\*
StyleBox[\"basis\", \"TI\"]\).";
MIs::usage="MIs[\!\(\*
...
nm /: EMs[nm] = ems;
nm/:Toj[nm]=toj;
nm/:Parameters[nm]=pars;
nm/:jsOrder[nm]=OptionValue[jsOrder];
jSector[nm]=js@@Prepend[ConstantArray[1,dim],nm];
MIs[nm]^={};Quiet[nm/:ToMyMIs[nm]=.];
(*Doubtful*)
...
LiteRedPrint[#<>" is valid basis."<>(If[save=!=False," The definitions of the basis will be saved in \""<>save<>"\" directory.",""])<>"\n    Ds[" <> # <>"] \[LongDash] denominators,\n    SPs[" <> # <>"] \[LongDash] scalar products involving loop momenta,\n    LMs[" <> # <>"] \[LongDash] loop momenta,\n    EMs[" <> # <>"] \[LongDash] external momenta,\n    Parameters[" <> # <>"] \[LongDash] parameters (invariants, masses, dimension),\n    Toj[" <> # <>"] \[LongDash] rules to transform scalar products to denominators,\n    CutDs[" <> # <>"] \[LongDash] flag vector of cut denominators."] &@ToString[nm];
...
MIs/:Set[MIs[nm_,inds:(0|1)..],mis_List]:=(MIs[nm]^=jVars[{DeleteCases[MIs[nm],_?(jSector[#]===js[nm,inds]&)],mis}(*,Sort\[Rule]jSector*)]);
...
GraphToAmplitude::usage="GraphToAmplitude[graph,{l1,l2,\[Ellipsis]}] constructs the amplitude from graph.\ngraph is a list of edges given in the form {n1->n2,type,[momemtum]}, where n1 and n2 are the numbers of vertices. Nonpositive n1 or n2 corresponds to external legs.\nOptions:\n    EdgeStyle->RightArrow \[LongDash] which head to use for edges,\n    Gather->Except[0] \[LongDash] which edges to gather as fermionic lines\n    EdgeShapeFunction->{_->Line} \[LongDash] which function to use for edges\n    VertexShapeFunction ->{_->Point} \[LongDash] which function to use for vertices\nExample:\n\!\(\*
StyleBox[\"GraphToAmplitude\",\nFontWeight->\"Bold\"]\)\!\(\*
StyleBox[\"[\",\nFontWeight->\"Bold\"]\)\!\(\*
StyleBox[\"{\",\nFontWeight->\"Bold\"]\)\!\(\*
StyleBox[\"{\",\nFontWeight->\"Bold\"]\)\!\(\*
StyleBox[\"1\",\nFontWeight->\"Bold\"]\)\!\(\*
...
Options[GraphToAmplitude]={EdgeStyle->RightArrow,Gather->Except[0],EdgeShapeFunction->{_->Line},VertexShapeFunction ->{_->Point}};
GraphToAmplitude[graph_,lms_,OptionsPattern[]]:=Module[
{vs,es=First/@graph,moms={},moms1,ngraph,eqs,sol,ampl,gtype=OptionValue[Gather],ef,vf,esf=OptionValue[EdgeShapeFunction], vsf=OptionValue[VertexShapeFunction]},
vs=Union@@(List@@@es);
ngraph={OptionValue[EdgeStyle]@@#1,{#3},#2}&@@@CompleteMomentaFlow[graph,lms];
ampl=ReplaceRepeated[Times@@(vf/@vs) Times@@ef@@@ngraph,ef[RightArrow[ns1__,k_],{ps1__},t:gtype]*ef[RightArrow[k_,ns2__],{ps2__},t:gtype]:>ef[RightArrow[ns1,k,ns2],{ps1,ps2},t]]/.{ef->(Replace[#3,esf][##]&),vf->(Replace[#,vsf][##]&)}
];
...
Options[DiskSave]={Directory->Automatic,Save->All,Replace->True,Quiet->True};
...
If[OptionValue[Replace],Off[DiskSave::overwrite]];
Replace[OptionValue[Save],
{All|Max:>(t="jRules[*"|"jExtRules[*"|"jSymmetries[*"|"jSymMatrices[*"|"MIs[*"|"MyMIs[*"|"ToMyMIs[*"|"ZeroSectors[*"|"NonZeroSectors[*"|"SimpleSectors[*"|"BasisSectors[*"|"UniqueSectors[*"|"MappedSectors[*"|"IBP[*"|"LI[*"|"SR[*";tb=True),
"Basis"|Min:>(tb=True),
...
If[MemberQ[x,"jRules"],t=t|"jRules[*"|"jExtRules[*"|"jSymmetries[*"|"jSymMatrices[*"|"MIs[*"|"MyMIs[*"|"ToMyMIs[*"];
...
{All|Max:>(t="jRules[*"|"jExtRules[*"|"jSymmetries[*"|"jSymMatrices[*"|"MIs[*"|"MyMIs[*"|"ToMyMIs[*"|"ZeroSectors[*"|"NonZeroSectors[*"|"SimpleSectors[*"|"BasisSectors[*"|"UniqueSectors[*"|"MappedSectors[*"|"IBP[*"|"LI[*"|"SR[*"),
"jRules":>(t="jRules[*"|"jExtRules[*"|"jSymmetries[*"|"jSymMatrices[*"|"MIs[*"|"MyMIs[*"|"ToMyMIs[*"),
"Sectors":>(t="ZeroSectors[*"|"NonZeroSectors[*"|"SimpleSectors[*"|"BasisSectors[*"|"UniqueSectors[*"|"MappedSectors[*"),
"Identities":>(t="IBP[*"|"LI[*"|"SR[*"),
...
polyjQ=If[TrueQ[OptionValue@PolynomialQ],TrueQ[(#===0||jVars[#(*Modified 16.03.2021*),Sort->False(*/Modified 16.03.2021*)]=!={})&&PolynomialQ[#,jVars[#(*Modified 16.03.2021*),Sort->False(*/Modified 16.03.2021*)]]&&(Not[TrueQ[OptionValue[SectorsPattern]]]||And@@(MatchQ[Rest[List@@#],SectorsPattern[First@#]]&/@DeleteDuplicates[jSector/@jVars[#(*Modified 16.03.2021*),Sort->False(*/Modified 16.03.2021*)]]))]&,True&];
...
toj[nm_?DsBasisQ,ex_]:=Module[{expr=Together[LFDistribute[ex,sp]],den}, den=Factor[Denominator[expr]/.Toj[nm]]; Expand[(j[nm,Sequence@@(0&/@Ds@nm)]/den)*(Numerator[expr]/.Toj[nm])]
];
...
(*-1 stands for cut denominators!!!*)
MakeOrderMatrix[sec:{(0|1|-1)...}]:=MakeOrderMatrix[sec,{"np","cp","-ds","-ns"}]
MakeOrderMatrix[sec1:{(0|1|-1)...},spec_List]:=Module[{
l=Length[sec1],
sec=Abs[sec1],
m,
unfold,
...
m={2sec-1};(*First criterion is total power, obligatory! See however "" below*)
...
"-np":>AppendTo[m,1-sec],
"cp"|"+cp":>AppendTo[m,-Replace[sec1,0|1->0,{1}]],
"-cp":>AppendTo[m,Replace[sec1,0|1->0,{1}]],
...
m=Fold[If[MatrixRank[Append[#1,#2]]>Length[#1],Append[#1,#2],#1]&,{},m];
(*Matrix should be constructed, but we need to check consistency and trim extra lines*)
If[Length[m]<l,Message[MakeOrderMatrix::bad,m]];
m
]
...
jsOrder[nm_,ns:(0|1)...]:=(nm/:jsOrder[nm,ns]=MakeOrderMatrix[js[nm,ns],Replace[jsOrder[nm],_jsOrder:>Sequence[]]]);
...
Return[(jc=!=jc0)&&MatchQ[Drop[jc,3],{__Integer}]&&OrderedQ[{jc,jc0}]]]
j/:j[nm1_,inds1__]>j[nm2_,inds2__]:=Module[{jc=jComplexity[j[nm1,inds1],j[nm2,inds2]],jc0},jc0=0&/@jc;
Return[(jc=!=jc0)&&MatchQ[Drop[jc,3],{__Integer}]&&OrderedQ[{jc0,jc}]]]
j/:j[nm1_,inds1__]<=j[nm2_,inds2__]:=Module[{jc=jComplexity[j[nm1,inds1],j[nm2,inds2]],jc0},jc0=0&/@jc;
Return[MatchQ[Drop[jc,3],{__Integer}]&&OrderedQ[{jc,jc0}]]]
j/:j[nm1_,inds1__]>=j[nm2_,inds2__]:=Module[{jc=jComplexity[j[nm1,inds1],j[nm2,inds2]],jc0},jc0=0&/@jc;
Return[MatchQ[Drop[jc,3],{__Integer}]&&OrderedQ[{jc0,jc}]]]
...
(*Modified 16.06.2021*)Options[jVars]={Sort->jComplexity};(*/Modified 16.06.2021*)
...
{
None|False:>DeleteDuplicates[Cases[{expr},_j,\[Infinity]]],
(*Added 16.06.2021*)jComplexity:>SortBy[DeleteDuplicates[Cases[{expr},_j,\[Infinity]]],jComplexity],(*/Added 16.06.2021*)
...
Scan[If[OrderedQ[{ReplacePart[Expand[jc-(jc1=jComplexity@#)],1->Signature[{First@jc1,First@jc}]],jc0}],jv=#;jc=jc1]&,Rest@jvars];
jv
]]
...
If[jvars==={},0,
ji=1;jc=jComplexity@First@jvars;jc0=ConstantArray[0,Length@jc];MapIndexed[If[OrderedQ[{ReplacePart[Expand[jc-(jc1=jComplexity@#)],1->Signature[{First@jc1,First@jc}]],jc0}],ji=1+First@#2;jc=jc1]&,Rest@jvars];
ji
]];
...
jc0=ConstantArray[0,Length@jc];
MapIndexed[If[OrderedQ[{jc0,ReplacePart[Expand[jc-(jc1=jComplexity@#)],1->Signature[{First@jc1,First@jc}]]}],ji=1+First@#2;jc=jc1]&,Rest@jvars];
ji
]];
...
StyleBox[\"basis\", \"TI\"]\),\[Ellipsis]]] gives the distance of the integral from the corner point of its sector.";


jLevel[j[nm_,ns__]]:=Plus@@Abs[{ns}-Rest[List@@jSector[j[nm,ns]]]]
...
StyleBox[\"basis\", \"TI\"]\)] generates IBP identities in parametric representation for unique sectors of the \!\(\*
StyleBox[\"basis\", \"TI\"]\).\n\
GenerateFPIBP[js[\!\(\*
StyleBox[\"basis\", \"TI\"]\),\!\(\*
StyleBox[\"indices\", \"TI\"]\)]] generates IBP identities in parametric representation for one sector.";
GenerateFPIBP::us="Calling GenerateFPIBP[`1`] requires UniqueSectors[`1`]";
GenerateFPIBP::failed="Failed to construct syzygy for the sector `1`";


Options[GenerateFPIBP]={DiskRecover->True,MonomialOrder->Automatic,TimeConstrained->False,
Append->{}(*extra variables (for speed, some parameters can be added here as vars)*),
...
{ffn,fn,
G,dG,xs,ns,
L=Length@LMs@nm,\[Nu]=MetricTensor[]/2,
syz,tsyz,Qk,Q,
jf,b,f,
mo=OptionValue[MonomialOrder],
rcv=OptionValue[DiskRecover],
attempts=OptionValue[Throw],
vs=OptionValue[Append],
...
dG=D[G,#]&/@xs;
If[mo===Automatic,
syz=Catch[TimeConstrained[
LiteRedPrintTemporary["Chose ordering: ",PermutationList[#,Length@xs]];
Quiet[tsyz=syzygy[Append[dG,G],Join[Permute[xs,#],vs]]];
If[tsyz=!=$Failed,Throw[tsyz]],tc,$Failed]&/@Prepend[RandomPermutation[Length@xs,attempts-1],Cycles[{}]];$Failed];
,
syz=TimeConstrained[syzygy[Append[dG,G],Join[xs,vs],MonomialOrder->mo],tc,$Failed];
];
If[syz===$Failed,Message[GenerateFPIBP::failed,js[nm,inds]];Return[$Failed]];
LiteRedPrintTemporary["Found "<>ToString[Length[syz]]<>" equations"];
...
f=f[Function[syz,
Qk=CoefficientRules[syz,xs];Q=Last@Qk;Qk=Most@Qk;
...
todo["GenerateFPIBP: add option DiskSave (as of now, it is always supposed to be true)"];


todo["LiteRed`Private`syzygy: implement minbase"];
...
FPIBP::usage="FPIBP[\!\(\*
StyleBox[\"basis\", \"TI\"]\),\!\(\*
StyleBox[\"indices\", \"TI\"]\)] returns a list of IBPs for the Feynman parametrization. All \!\(\*
StyleBox[\"indices\", \"TI\"]\) should be nonnegative, otherwise FPIBP returns {}.\n\
FPIBP[\!\(\*
StyleBox[\"basis\", \"TI\"]\)][\!\(\*
StyleBox[\"indices\", \"TI\"]\)]\!\(\*
StyleBox[\" \", \"TI\"]\)evaluates to the former form.";
FPIBP::nonum="Numerators are not allowed.";


FPIBP[nm_][inds__]:=FPIBP[nm,inds]
...
StyleBox[\"sector\", \"TI\"]\).\nOptions:\n\
    ...
    NMIs\[Rule]n:-1, expected number of master integrals. If the number of the integrals left is less or equal to n, the program terminates assuming those integrals are masters, which may save some time.\n\
...
Options[SolvejSector]={
Depth->2(*minimal search depth*),
MaxDepth->\[Infinity](*maximal search depth*),
Level->-1(*Level n leads to saving partial rules with n symbolic indices*),
NMIs->-1(*expected number of masters, -1 means elaborate thoroughly*),

RRs->Automatic,(*relations to use, Automatic or _Function*)
SR->True,(*use symmetries?*)(*as of now, only for numeric indices*)
jRules->({{}}&),

DiskSave->True(*whether to save rules on the disk?*),
...
First->Automatic,(*SolvejSector[nm,First\[Rule]js[nm,\[Ellipsis]]] is a shortcut for SolvejSector[Drop[#,Replace[Position[#,js[nm,\[Ellipsis]],{1},1],{{n_}\[RuleDelayed]n-1,{}\[Rule]-1}]]&@UniqueSectors[nm],DiskRecover\[Rule]True] *)
After->Automatic,(*SolvejSector[nm,After\[Rule]js[nm,\[Ellipsis]]] is a shortcut for SolvejSector[Drop[#,Replace[Position[#,js[nm,\[Ellipsis]],{1},1],{{n_}\[RuleDelayed]n,{}\[Rule]-1}]]&@UniqueSectors[nm],DiskRecover\[Rule]True] *)
GeneratedCell->True,
NamingFunction->Automatic(*index naming*),
jGraph->False
...
us=Drop[us,drop];
res=SolvejSector[us,opts];
If[drop==0&&MatchQ[res,{__Integer?Positive}],CurrentState[nm,SolvejSector]=True];
res
]
SolvejSector[sects_List,opts:OptionsPattern[]]:=SolvejSector[#,opts]&/@sects


todo["Parameters: make fallback for old-style bases"];


SolvejSector[jsect:js[nm_,ns:(0|1)...],OptionsPattern[]]:=Module[{
file=StringReplace[ToString@jsect,"js"->"jRules"],
(*(*Deleted 18.12.2020*)nm=First@jsect,(*/Deleted 18.12.2020*)*)
(*Modified 18.12.2020*)
corner={ns}(*Rest[List@@jsect]*),(*/Modified 18.12.2020*)
indices,
constr,
(*Modified 18.12.2020*)
nds=Length@{ns}(*Length@jsect-1*),(*/Modified 18.12.2020*)
ps,
reserved=False,
indicnop=0,
indicnopToGo=0,
...
noRules,
gatherRules,
rulesFound={},ruleFound,
misFound={},
parameters,
eqs,sf,
dbase(*jRulesDB*),
ids,sharp=If[TrueQ[OptionValue[Sharpen]],sharpen,Identity],
cases,case,ii,
rules2,
startp,startps,
contiguous,vars,inds,
pat1,except,usefer,
ptrnrule,jRules1,jRulesF,pos,searchDepth,level,useSR,indicnr,disksave,diskreco,onmis,
tc,tcf,tct,tctr,tc1,
sjopts,whenBad,smartReduce,(*cf,
fromRules,expandRules,*)found,depth,maxDepth,
pointsdone={},pointsprepared,
colorize,ordering,
badconditions,jsorder,throw,
init,clean,submit,solve,collect
},
...
ids=Replace[OptionValue@RRs,{Automatic->IBPLI[nm],x:IBPLI|IBP|LI|FPIBP:>x[nm]}];
If[usefer,
init=finitdb;clean=fcleandb;submit=fsubmiteqs;solve=fsolveeqs;collect=#&,
init=initdb;clean=cleandb;submit=submiteqs;solve=solveeqs;collect=Collectj[#,sf]&
];
(*/ Processing options *)
...
indicstime=AbsoluteTime[];
ordering=Ordering[(Transpose[jsOrder[nm,ns]])*(2corner-1)];
colorize=Function@@({MapThread[Style[#1,ColorData["RustTones"][#2]]&,{Slot/@Range[Length@corner],Permute[Range[Length@#]/Length@#,#]}]}&[ordering]);
(*Modified 03.03.2021*)
gatherRules[norules_]:=(*List/@Flatten[#,1]&@*)SortBy[Sort[#,(Last@#1)>(Last@#2)&]&/@Gather[{#,j[nm,##]&@@indices/.#}&/@norules,MatchQ[Expand[List@@Last@#1-List@@Last@#2],{__Integer}]&(*Gather contiguous*)],{
Length@First@First@#,(*# of fixed parameters*)
Count[Last@First@#,_Integer],(*# of numeric indices*)
(*(*Deleted 03.03.2021*)Flatten@Position[Take[jComplexity[Last@First@#],-nds],Except[_Integer],{1}](*/Deleted 03.03.2021*)*)
Flatten@Position[Rest[List@@Last@First@#][[ordering]],Except[_Integer],{1},Heads->False]
(*position of symbolic indices*)
}&][[All,All,1]];(*/Modified 03.03.2021*)
If[$LiteRedMonitor,moni=PrintTemporary["    ",TableForm[{{Dynamic[ToString[indicnr]<>" point: spent "<>ToString[Round[AbsoluteTime[]-indicstime]]<>" seconds"<>indicsip<>".",UpdateInterval->1]},Dynamic[Overlay[{ProgressIndicator[indicpb,{0,nds+1/2}],ToString[indicnop]<>"\[Rule]"<>ToString[indicnopToGo]},Alignment->Center]],Dynamic[colorize@@@startps]}]]];
...
vars=(*before 06.11.2025*)(*Join[parameters,inds];*)(*/before 06.11.2025*)(*Modified 06.11.2025*)Join[inds,parameters];(*/Modified 06.11.2025*)
clean[dbase,vars];
...
If[!numeric||Length[misFound]+Total[Length/@noRules]+Length[cases]>onmis,
(* check if each case is a subcase*)
(*If[(ruleFound=Select[rulesFound,(ii=Flatten@Position[(TrueQ[#1]&&Not[TrueQ[Simplify[#2]]])&@@@(#/.cases),True,{1},1])=!={}&,1])=!={},
{case}=cases[[ii]];
AppendTo[badconditions,RulesToCondition[{case}]&&ruleFound[[1,2]]];cases=Delete[cases,First@ii];
]*)
ii=MapIndexed[
Function[{case,i},If[(ruleFound=Select[rulesFound,(TrueQ[#1]&&Not[TrueQ[Simplify[#2]]])&@@(#/.case)&,1])=!={},
AppendTo[badconditions,RulesToCondition[{case}]&&ruleFound[[1,2]]];i,Unevaluated[Sequence[]]]
],
cases];
...
If[cases==={},found=True;Break[]];
If[numeric&&Length[misFound]+Total[Length/@noRules]+Length[cases]<=onmis,Break[]];
(*Construct equations*)
eqs=Flatten[(If[useSR&&numeric,Join[ids@@#,SR[nm]@@#]/.ZerojRule[nm],ids@@#])&/@(DeleteCases[pointsprepared=preparepoints[jsect,If[numeric,startps,startp],depth],Alternatives@@pointsdone])];
...
jRules1=(#/.MapThread[#1->Expand[2#1-#2]&,{indices[[pos]],Rest[List@@First[#]][[pos]]}])&[ruleFound];
...
If[cases==={},found=True;Break[]];
If[++depth>searchDepth,If[depth<=maxDepth&&!numeric,searchDepth=depth;AppendTo[indicoutput,LiteRedPrintTemporary[Style["Increasing search depth to "<>ToString[searchDepth],Tiny]]],
Break[]
(*(AppendTo[indicoutput,LiteRedPrint(*Temporary*)["Found master integral: "<>ToString[startp]]];AppendTo[misFound,j[nm,##]&@@startp]);
cases=Rest[cases];depth=0*)
]];
(*If[cases==={},found=True;Break[]];*)
If[startp=!=First[startps],startp=First[startps];depth=0];
pointsdone=Join[pointsdone,pointsprepared]
(*/ depth\[LessEqual]searchDepth*)]
];
If[!found,
...
(*/ noRules=!={}*)];
If[$LiteRedMonitor,NotebookDelete[moni]];(*(*Deleted 10.06.2021*)numeric=True;(*/Deleted 10.06.2021*)*)
,tc,Unevaluated[LiteRedPrint[Style["Exceeded time limit ("<>ToString[tc]<>") for solving the sector. Exiting SolvejSector with $Failed\[Ellipsis]",Red]];throw=True;Throw[$Failed]]]
];
...
Quiet[nm/:ToMyMIs[nm]=.];
If[level>=0,
misFound={};LiteRedPrint["    "<>ToString["jRules"@@jsect]<>" \[LongDash] partial reduction rules for the sector."]
,
...
cleandb[dbase_,vars_]:=(ToExpression[dbase[[1]]<>"={0\[Rule]0};"];dbase[[-1]]={});
...
preparepoints[jsector_js,startps:{__List},depth_]:=Module[{
nm=First@jsector,
...
dnpos=Flatten[Position[startps[[1]],_Integer,{1}]];
points=Rest[List@@#]&/@Sort[Union@@(Function[startp,(Select[j[nm]@@(startp+#)&/@diamond[l,depth],jSector[#]===jsector&]
)]/@startps),Less];
points
]


preparepoints[jsector_js,startp:{Except[_List]..},depth_]:=Module[{
nm=First@jsector,
corner=Rest[List@@jsector],
points,jso=jsOrder@@jsector,
l=Length[jsector]-1,
...
dnpow=startp[[dnpos]];
points=Rest[List@@#]&/@Sort[j[nm]@@(startp+#)&/@Select[diamond[l,depth],UnitStep[#[[dnpos]]+dnpow-1/2]===corner[[dnpos]]&],Less];
points
]
...
BiggestSectors::usage="BiggestSectors[\!\(\*
StyleBox[\"basis\", \"TI\"]\),\!\(\*
StyleBox[\"pattern\", \"TI\"]\)] returns the biggest sectors of the \!\(\*
StyleBox[\"basis\", \"TI\"]\)\!\(\*
StyleBox[\" \", \"TI\"]\)(which also can be a \!\(\*
StyleBox[\"set\", \"TI\"]\)) matching the \!\(\*
StyleBox[\"pattern\", \"TI\"]\).\nIf \!\(\*
StyleBox[\"pattern\", \"TI\"]\) is omitted, SectorsPattern[\!\(\*
StyleBox[\"basis\", \"TI\"]\)] is used instead.";


BiggestSectors[nm_,patt_:Automatic]:=Module[{big,sectors,n=NDs[nm],pat=Replace[patt,Automatic->SectorsPattern[nm]]},
big={};
sectors=Select[Range[0,2^n-1],MatchQ[IntegerDigits[#1,2,n],pat]&];
While[sectors=!={},(AppendTo[big,#];
sectors=DeleteCases[sectors,x_/;(BitOr[x,#]===#)])&[Last[sectors]]];
js[nm]@@IntegerDigits[#1,2,n]&/@big
]
...
FindShifts::opt="Something wrong in option `1`. Ignoring it.";
...
ppairs=Replace[Subsets[psx,{1,2}],{x_}:>{x,x},{1}];eqs=Reap[Collect[LFDistribute[sp@@@(ppairs/.Thread[psx->psxn]),sp]-(sp@@@ppairs),Variables[{MetricTensor[],sp@@@ppairs}],Sow]][[-1,1]];Quiet[Check[Sort[Thread[psx->psxn]/.Solve[0==GroebnerBasis[eqs,Flatten@psmxn],Flatten@psmxn]],Message[FindShifts::opts,All];{{}},Solve::svars]]
),
False|None->{{}},
e_:>(
ppairs=Replace[Subsets[psx,{1,2}],{x_}:>{x,x},{1}];
If[TrueQ[MatchQ[Union@@(ExpandAll[LFDistribute[sp@@@(ppairs/.#),sp]-(sp@@@ppairs)]&/@e),{0}]],e,Message[FindShifts::opts,e];{{}}])
}];
...
SR[nm_?DsBasisQ,ns__]:=SR[nm][ns]


SectorsMappings[jjs_js]:=SectorsMappings@@jjs;


Options[FindSymmetries]={
NamingFunction->Automatic,
EMs->True,
GroebnerBasis->True,
ExtUniqueSectors->True
};
...
us,ujs,es,jsec,s,subs,t,l,sol,i,solve},
...
solve=If[TrueQ[OptionValue[GroebnerBasis]],Solve[0==GroebnerBasis[#1,#2],#2]&,Solve[0==#1,#2]&];
ps=Replace[PowerShifts[nm],Except[0]->1,{1}];
...
gle=Array[m,{Length@lms,Length@lms+Length@ems}];Declare[Evaluate@(Flatten@gle),Number];
egle=Array[m,{Length@ems,Length@ems},Length@lms+1];Declare[Evaluate@(Flatten@egle),Number];
...
If[xeqs=!={},
xeqs=GroebnerBasis[Reap[Collect[Numerator@Factor[xeqs],Select[Variables[{MetricTensor[],sp@@@Subsets[ems,{2}],LFDistribute@Ds@nm}],FreeQ[#,Alternatives@@lms]&],Sow]][[-1,-1]],Flatten@egle]];
...
Do[
Outer[(
sol=Check[solve[Flatten@{#1[[#2]]-s[[k,1]],xeqs},gle],
Message[FindSymmetries::ems];Abort[]
,{Solve::svars}];
Function[sr(*solution rules*),matr=#1/.sr;
If[Factor[cds*matr . cds]===cds&&shiftcompQ[matr],
...
Select[m/.#&/@Quiet[solve[Flatten@{m[[tp]]-t,xeqs},gle]],Factor[cds*# . cds]===cds&&shiftcompQ[#]&]],dmatr,top,1],2]](*self shifts*),
...
LiteRedPrintTemporary["    Found "<>ToString[Length@ss]<>" unique simple sectors with multiplicities "<>StringRiffle[ToString@*Length@*First/@ss,","]<>" respectively."];
...
s={ujs};
es=DeleteCases[(
If[(*Modified 10.01.2024*)(*MemberQ[nzs,#1]*)MatchQ[Rest[List@@#1],SectorsPattern[nm]](*/Modified 10.01.2024*)&&!MemberQ[s,#1],
...
Options[FindExtSymmetries]={NamingFunction->Automatic,EMs->False,GroebnerBasis->True};
...
jsm=js2;{js2,#1,#2[[All,All,1]]}&@@polyNF[fp/.Thread[Pick[xs,List@@Rest@js2,0]->0],Pick[xs,List@@Rest@js2,1]]]/@us2,{js1_,sgn_,prm_List}:>(Sequence@@({js1,sgn,#}&/@prm)),{1}](*/Modified 13.08.2019*),
...
(jsm=sec->First@#;
Do[
sol=solve[Flatten[dmatr[[k,Last@pnf]]-(PadRight[PadLeft[{1},#],Length@ds2+1]&/@Last@#)],Flatten[cs]];
Function[sr,
...
MIs[nm]^=jVars[Complement[MIs[nm],t](*(*Deleted 16.03.2021*),Sort\[Rule]jSector(*/Deleted 16.03.2021*)*)];Quiet[nm/:ToMyMIs[nm]=.];
...
AddjRule[jj_j:>rhs_,opts:OptionsPattern[]]:=AddjRule[jj->rhs,opts];
...
MIs[nm,inds]=mis;Quiet[nm/:ToMyMIs[nm]=.];
mis
];
...
MyMIs::usage="MyMIs[basis] returns the list of custom master integrals.";
ToMyMIs::usage="ToMyMIs[basis] returns the the rules for passing to custom master integrals MyMIs[basis].";
MyMIs::lindep ="Custom master integrals should be linearly independent.";
...
MyMIs/:Set[MyMIs[nm_?DsBasisQ],mislist_List]:=Module[{mis=mislist,tomis},If[(tomis=ToMIsRule[mis])=!=$Failed,
If[Length[mis]<Length[MIs[nm]],Message[MyMIs::append];mis=jVars[MIs[nm]/.tomis]];
MyMIs[nm]^=mis;
ToMyMIs[nm]^=tomis;
LiteRed`Private`LiteRedPrint["Successfully set custom master integrals."];mis,
Message[MyMIs::lindep];$Failed
]
]


MyMIs/:Set[MyMIs[nm_?DsBasisQ,ns:(0|1)..],mislist_List]:=Module[{mis=MIs[nm,ns],tomis},
If[Length[mislist]!=Length[mis]||MatrixRank[Outer[Coefficient,IBPReduce[mislist,jSector->js[nm,ns]],mis]]<Length@mis,Message[MyMIs::lindep];$Failed,
MyMIs[nm]^=Replace[MyMIs[nm],Thread[mis->mislist],{1}];
Quiet[nm/:ToMyMIs[nm]=.];
mislist
]
]
...
MyMIs[nm_?DsBasisQ,ns:(0|1)..]:=Select[MyMIs[nm],Union[jSector/@jVars[#]]==={js[nm,ns]}&]


MyMIs[nm1_?DsBasisQ,nms__?DsBasisQ]:=Join@@(MyMIs/@{nm1,nms});


ToMyMIs[nm_?DsBasisQ]:=LiteRed`Private`LiteRedMonitor[ToMyMIs[nm]^=ToMIsRule[MyMIs[nm]],Evaluate["Calculating ToMyMIs["<>ToString[nm]<>"]..."]]


ToMyMIs::order="Arguments of ToMyMIs are expected to be ordered.";


ToMyMIs[nm1_?DsBasisQ,nms__?DsBasisQ]:=(If[!OrderedQ[{nm1,nms}],Message[ToMyMIs::order]];Fold[Join[#1,#2/.Dispatch[#1]]&,ToMyMIs/@{nm1,nms}])
...
IBPSelect::exists="File `1` already exists. This may happen if the vanishing coefficients are not simplified to zero, e.g. 2+2x-2(1+x). Try SetOption[IBPSelect,Simplify->Together]."


Options[IBPSelect]={jExtRules->True,jRules->jRules,jSector->_,Simplify->Together};
...
extrules=OptionValue[jExtRules];(*\:0441\:043e\:0431\:0438\:0440\:0430\:0435\:043c \:0432\:0441\:0435 j, \:043e\:043f\:0440\:0435\:0434\:0435\:043b\:044f\:0435\:043c \:0441\:043b\:043e\:0436\:043d\:0435\:0439\:0448\:0438\:0439 \:0441\:0435\:043a\:0442\:043e\:0440*)
...
Message[IBPSelect::exists,ToString[hjsec]];
...
If[OptionValue[Simplify]=!=Identity,nrs=Collect[nrs,_j,OptionValue[Simplify]]];(*/Modified 06.11.2025*)
...
(*on which sectors depends hjsec*)
...
todo["Tunnel option Simplify of IBPSelect from IBPReduce"];
todo["Write a specialized procedure to normalize jRules so that IBPSelect could work with option Simplify->Identity"];
...
Options[IBPReduce]={jExtRules->True,jSector->_,jRules->jRules,ToMyMIs->False,Collect->Collectj,Directory->Automatic,Fermatica`UseFermat->False};


IBPReduce[expr_,OptionsPattern[]]:=Module[
{dir=OptionValue[Directory],dir1,i=0,res,
jsd,
jsec,hjsec,
t1,t2,jrules,jrules1,
c,subs,
mis,
nr=0,nr1=0,level,
status,
...
(*The line below assumes that right-hand sides of jrules contain either masters or the integrals of subsectors*)
...
FermatIBPReduce[ex_,OptionsPattern[]]:=Module[{dir,i,ex1,mis,jvars,j2n,tomis,jsecs,nrt,reap,resrules={},jrules,vrules,matr,program,fermat,nr=0,jvs,res,save},
...
DeleteDirectory[dir,DeleteContents->True]];
Monitor[
matr="Array mat["<>ToString[nrt]<>","<>ToString[Length[jvars]]<>"] Sparse;\n[mat] := [ "<>StringRiffle[MapIndexed[Function[{rule,i},jvs=DeleteDuplicates@Cases[rule,_j,-1];StringReplace[ToString[Join[i,SortBy[List@@@Thread[(jvs/.j2n)->Coefficient[-Subtract@@rule,jvs]](*Transpose[{jvs/.j2n,Coefficient[-Subtract@@rule,jvs]/.vrules}]*),First]]/.vrules,InputForm],{"{"->"[ ","}"->"] "}]],jrules],"\n"]<>"];","Constructing sparse matrix..."];
...
SparxIBPReduce::usage="SparxIBPReduce[\!\(\*
StyleBox[\"expr\", \"TI\"]\)] performs the reduction of the \!\(\*
StyleBox[\"expr\", \"TI\"]\) applying rules found by FindSymmetries and SolvejSector.";


Options[SparxIBPReduce]={Run->True};


SparxIBPReduce[ex_,OptionsPattern[]]:=Module[{place,in,out,output={""},i,ex1,mis,jvars,jsecs,nrt,nr=0,reap,resrules={},jrules,vrules,flint,l,prules},
If[!MemberQ[$ContextPath,"Sparx`"|"FLINT`"],Print["Sparx` package not found"];Return[$Failed]];
i=1;While[FileExistsQ[place="IBPSelection"<>ToString[i]],i++];
...
If[TrueQ[OptionValue[Run]],
jrules=Symbol["SparxGaussSolve"][Subtract@@@jrules,jvars,First/@resrules,Forward->False];
...
prules=Symbol["SparxPutEquations"][Subtract@@@jrules,jvars,in];
OpenAppend[in];
...
StyleBox[\"put\",\nFontWeight->\"Bold\"]\), e.g., in the file \""<>out<>"\".\nThen use SparxGetRules[\""<>out<>"\",jvars]/.(prules), where {jvars,prules} is the output below."];
DeleteDirectory[place,DeleteContents->True];
...
Options[ToMIsRule]={Fermatica`UseFermat->False}
...
Return[DeleteCases[Thread[mis->If[OptionValue[Fermatica`UseFermat],Fermatica`FInverse[matr],Factor[Inverse[matr]]] . nmis],HoldPattern[a_->a_]]]
]
...
t2=\[Sigma] Together[(dt2*den/.Thread[lms->0])-LFDistribute[Inner[sp,t1,adjugate[t2] . t1,Plus],_sp]];
...
FeynParUF[dsl_List,lms_List,n_List,opts:OptionsPattern[]]:=Module[
...
{Gamma[Plus@@n-l*d/2]/Times@@Gamma/@DeleteCases[n,_?NonPositive]*(Times@@((#1^(#2-1))&@@@(Cases[Transpose[{xs,n}],{_,_?(!TrueQ[NonPositive[#]]&)}])))r,Pick[xs,Not/@TrueQ/@NonPositive/@n]}
]
...
StyleBox[\"F\", \"TI\"], \"i\"]\), \(i\)]\)\!\(\*UnderscriptBox[\(\[Product]\), \(j\:f88c \[NotEqual] i\)]\)\!\(\*SubscriptBox[
StyleBox[\"U\", \"TI\"], \(i\)]\).";
...
Options[MakeDSystem]={IBPReduce->IBPReduce}
...
If[Head[x]===List,
eqs=OptionValue[IBPReduce][Dinv[mis,#]&/@x]/.tomis;
...
If[Head[x]===List,Thread[x->Mi],x->Mi]
]
...
MIs[nm]^=jVars[{DeleteCases[MIs[nm],_?(jSector[#]===sect&)],mis}(*(*Deleted 16.03.2021*),Sort\[Rule]jSector(*/Deleted 16.03.2021*)*)];
...
Options[jGraph]={Label->{Style,Dot,Number}(*can be any sane expression involving Style, Number, Dot, e.g. Label\[Rule]{Style,Dot,Number}. External legs always remain to be just a Label*)
(*Dot\[Rule]False*)(**),Left->{}(*which vertices come first*)};


jGraph[j[nm_Symbol,x__],OptionsPattern[]]:=Module[{graph=None,jsect=jSector[j[nm,x]],contracted,newv,tmp,edge,i=0,lab,he=(Rule|DirectedEdge|UndirectedEdge)},
...
graph=MapAt[{Style->#,Dot->0,Number->#}&,Last[#],{All,2}];
MapIndexed[(graph[[First@#2,2]]={Style->(Style/.graph[[First@#2,2]]),Number->First@#1,Dot->0})&,Position[Rest[List@@First@#],1]];
Throw[graph]])&,
jGraph[nm]
]
];
...
Scan[(graph=Function[{e,l},{e/.(Rule@@graph[[#,1]]),l}]@@@Delete[graph,#])&,contracted];
If[!FreeQ[lab,Dot](*OptionValue[Dot]*),
MapIndexed[(graph[[First@#2,2,3]]=Dot->#1-1)&,DeleteCases[{x},_?NonPositive]]
,
(*Now we need to add segments for dots*)
newv=Max[Sequence@@@(First/@graph)(*Cases[First/@graph,(_Rule|_DirectedEdge|_UndirectedEdge),\[Infinity]]*)]+1;
(*unique new vertex index*)
MapIndexed[(For[tmp=#1,tmp>1,tmp--,AppendTo[graph,MapAt[#/.{(h:he)[v1_,v2_]:>(h[newv,v2])}&,graph[[First[#2]]],{1}]];
graph[[First[#2],1]]=graph[[First[#2],1]]/.{(h:he)[v1_,v2_]:>(h[v1,newv++])};
...
GraphSort[graph,OptionValue[Left]]
]


GraphSort::usage="GraphSort[\!\(\*
StyleBox[\"graph\", \"TI\"]\),\!\(\*
StyleBox[\"start\", \"TI\"]\)] sorts \!\(\*
StyleBox[\"graph\", \"TI\"]\)\!\(\*
StyleBox[\"=\", \"TI\"]\)\!\(\*
StyleBox[\"{\", \"TI\"]\)\!\(\*
...
StyleBox[\",\", \"TI\"]\)\!\(\*
StyleBox[\"...\", \"TI\"]\)\!\(\*
StyleBox[\"}\", \"TI\"]\) with starting from the edges that contain the vertices in the \!\(\*
StyleBox[\"start\", \"TI\"]\)\!\(\*
StyleBox[\"=\", \"TI\"]\)\!\(\*
StyleBox[\"{\", \"TI\"]\)\!\(\*SubscriptBox[
StyleBox[\"v\", \"TI\"], \(i\)]\)\!\(\*
...
StyleBox[\",\", \"TI\"]\)\!\(\*
StyleBox[\"...\", \"TI\"]\)\!\(\*
StyleBox[\"}\", \"TI\"]\). This is an attempt to give user some control on how GraphPlot layouts graphs. Basically, the vertices in the \!\(\*
StyleBox[\"start\", \"TI\"]\) list are expected to appear to the left of the GraphPlot picture.";
GraphSort[jgraph_List,lverts_List]:=Module[{jgraph1=jgraph,jgraph2={},level,lverts1=Alternatives@@lverts},
(*Assign weight*)
While[jgraph1=!={},
level=Cases[jgraph1,{lverts1->_,___}|{_->lverts1,___}];If[level==={},jgraph2=Join[jgraph2,jgraph1];Break[]];
jgraph1=DeleteCases[jgraph1,Alternatives@@level];
lverts1=Alternatives@@Union@@List@@@First/@level;(*Print[lverts1];*)
jgraph2=Join[jgraph2,level];
];
jgraph2
]
...
jGraphFeynMP::usage="jGraphFeynMP[j[...]] produces feynmp graph. See Options[jGraphFeynMP]."


Options[jGraphFeynMP]={Left->Automatic,Right->Automatic,EdgeStyle->{_->"plain"},EdgeLabeling->False,ImageSize->{120,75},Frame->False,PlotLabel->False};


jGraphFeynMP[jj_j,OptionsPattern[]]:=Module[{jgraph=jGraph[jj],vs,left,right,edge,begin,part1,part2,part3,end,t=0,dn=0,frame,title,multirules},
left=Replace[OptionValue[Left],Automatic->Cases[jgraph[[All,1,1]],_?Negative]];
right=Replace[OptionValue[Right],Automatic->Cases[jgraph[[All,1,2]],_?Negative]];
vs=Complement[Union@@List@@@jgraph[[All,1]],left,right];
left=MapIndexed[#1->"i"<>ToString[First@#2]&,left];
right=MapIndexed[#1->"o"<>ToString[First@#2]&,right];
vs=MapIndexed[#1->"v"<>ToString[First@#2]&,vs];
{left,right,vs};
jgraph=MapAt[List@@#/.Join[left,right,vs]&,jgraph,{All,1}];
{part1,part2}=Reap["\\fmfleft{"<>StringRiffle[Last/@left,","]<>"}\n  "<>"\\fmfright{"<>StringRiffle[Last/@right,","]<>"}\n  "<>StringRiffle[(edge="\\fmf{"<>(#2[[1]]/.OptionValue[EdgeStyle])<>(*edge labelling*)",tag="<>ToString[++t]<>"}{"<>StringRiffle[#1,","]<>"}";If[#2[[2]]>0,Sow["\\fmfvsp["<>ToString[t]<>"]{"<>StringRiffle[#1,","]<>"}{"<>StringRiffle[Table["d"<>ToString[++dn],{#2[[2]]}],","]<>"}"]];edge)&@@@jgraph,"\n  "]];
multirules=Flatten[Function[edges,MapIndexed[",tag="<>ToString[#[[1]]]<>"}"->",tag="<>ToString[#[[1]]]<>",left="<>ToString[If[OrderedQ[#1[[2]]],1,-1]((First[#2]-1)/(Length[edges]-1)-0.5)]<>"}"&,edges]]/@Select[GatherBy[MapIndexed[{First@#2,#}&,jgraph[[All,1]]],Union[Last[#]]&],Length[#]>1&]];
part1=StringReplace[part1,multirules];
If[part2=!={},part2=StringRiffle[Flatten[part2],"\n  "]<>"\n  ",part2=""];
If[vs=!={},part3="\\fmfdot{"<>StringRiffle[Last/@vs,","]<>"}"];
If[dn!=0,part3=part3<>"\n  \\fmfdot{"<>StringRiffle[Array["d"<>ToString[#]&,dn],","]<>"}"];
begin="\\begin{fmfgraph*}("<>StringRiffle[ToString/@OptionValue[ImageSize],","]<>")\n  ";
end="\n\\end{fmfgraph*}";
If[MatchQ[frame=OptionValue[Frame],{{_,_},{_,_}}],
begin="\\fmfframe"<>StringReplace[StringJoin@@(ToString/@frame),{"{"->"(","}"->")"," "->""}]<>"{\n"<>begin;
end=end<>"\n}";
];
title=Replace[OptionValue[PlotLabel],True->StringReplace[ToString[jj,InputForm],","->",,"]];
If[title=!=False,
begin=begin<>"\\fmfbottom{ttl}\n  \\fmfv{l.a=-90,l=$"<>title<>"$}{ttl}\n  ";
];
begin<>part1<>"\n  \\fmffreeze\n  "<>part2<>part3<>end
]
...
MemberQ[$ContextPath,"Singin`"],$LiteRedSyzygy=Symbol["singMBSyz"],
...
ndiamond[l_Integer,d_Integer]:=Select[diamond[l,d],Total[#]<=0&]
...
If[$Notebooks,If[LiteRed`Private`template,
